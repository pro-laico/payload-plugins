...
/** `<Icon name="…" />` — reads the active icon set through a cached, tagged read, so a page full of
 * icons still prerenders.
 *
 * `draft` is a prop rather than a `draftMode()` call. Reading `draftMode()` here made every page
 * that renders a single icon dynamic forever, whether or not anyone was previewing — the cost of
 * preview support paid by every published page. A preview route passes it explicitly:
 * `<Icon name="cart" draft={(await draftMode()).isEnabled} />`. */
export const createIcon = (payload: Payload | Promise<Payload>): ((props: IconProps) => Promise<React.ReactElement>) => {
  return async function Icon({ name, fallback, draft = false, ...svgProps }: IconProps): Promise<React.ReactElement> {
