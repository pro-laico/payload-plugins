...
/**
 * Build your app's `<Icon>` component, seeded ONCE with your live Payload session —
 * package code never resolves Payload itself. Seed it at MODULE SCOPE (the per-request
 * one-query memo is keyed by the handle reference; a fresh handle per render costs K
 * queries for K icons):
 *
 * ```ts
 * // src/components/icon.ts — the one seam
 ...
 * ```
 *
 * The returned server component renders a CMS-managed icon by name: looks up the active
 * `iconSet`, finds the entry matching `name`, and inlines its `<svg>` with the source's
 * intrinsic attributes (viewBox, fill, …) merged under any JSX props you pass (so
 * `className`, `style`, `width`, etc. always win). It inherits CSS `color` via
 * `currentColor`. Falls back to {@link IconProps.fallback} (or a built-in warning glyph)
...
export const createIcon = (payload: Payload | Promise<Payload>): ((props: IconProps) => Promise<React.ReactElement>) => {
  return async function Icon({ name, fallback, ...svgProps }: IconProps): Promise<React.ReactElement> {
    ...
    if (!svg) {
      ...
      void warnIconMissDev(payload, name, draft) // dev-only, once per name — diagnoses WHY the name missed
    }
