 ...
 * handed the same slug the marker carries rather than assuming the default. `access` is the resolved
 * per-endpoint gate; unset falls through to any logged-in user. */
export function createClearIconRequestsEndpoint(slug: string, access: EndpointAccess | undefined): Endpoint {
  return {
    path: CLEAR_ICON_REQUESTS_PATH,
    method: 'delete',
    handler: async (req) => {
      try {
        if (!(await isAllowed(access, req))) {
          return Response.json({ success: false, message: 'Not authorized to clear icon requests.' }, { status: 401 })
        }
