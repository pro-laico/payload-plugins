/**
 * Pure hotspot/crop geometry — no Sharp, no Node APIs. Isomorphic on purpose: the
 * transform endpoint (server), the blurhash coefficient crop, and the admin focal
 * preview (client) all import it, so what the author sets against the preview is
 * exactly what the endpoint renders.
 *
 * Three stored, non-destructive layers compose into one crop window (Sanity's model):
 *  - **crop** — edge trims (percent per side): "only this region may ever be used"
 *  - **focal point** — `focalX`/`focalY` percentages: windows center here
 *  - **focal size** — the hotspot circle's diameter as a percent of the crop region's
 *    shorter side: how much context must stay in frame. 100 (the default) demands the
 *    maximum window — exactly the pre-hotspot behavior — while smaller values zoom in.
 */
...
/**
 * The cover-crop window for a target aspect ratio: the smallest rect at that ratio that
 * contains the hotspot circle, grown/clamped to the crop region and centered on the focal
 * point (drifting only at the region's edges). `focalSize: 100` yields the region's maximal
 * window — identical to plain focal-cover cropping.
 */
...
/**
 * The CSS placement that reproduces {@link hotspotWindow} inside a preview tile whose
 * aspect ratio equals the window's: percentages (relative to the TILE box) for an
 * absolutely-positioned `<img>` — `width` scales the source so the window fills the tile,
 * `left`/`top` shift the window's origin onto the tile's. Handles zoom and crop, which
 * plain `object-position` cannot express.
 */
...
/**
 * The CSS `object-position` percentages (x, y) that reproduce the DEFAULT-opts cover crop
 * for a box of the given aspect ratio (legacy helper — it cannot express zoom or crop;
 * prefer {@link windowCss}). Falls back to centered when an axis isn't cropped.
 */
