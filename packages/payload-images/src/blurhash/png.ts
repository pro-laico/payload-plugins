/**
 * BlurHash → PNG `data:` URI, in pure JS: decode the cosine series to a small pixel grid
 * and hand-assemble a PNG (8-bit RGB, filter 0, one zlib deflate) — no Sharp, no canvas.
 * This is how `<ResponsiveImage>` paints a placeholder with zero client JS: microseconds
 * of string/byte math on a hash the doc already carries. Server-only (node:zlib).
 */
...
/** Assemble a minimal 8-bit RGB PNG from raw pixel bytes (no alpha, no interlace). */
...
export interface BlurhashPngOptions {
  /** Rendered width in px. Default 32 — plenty for a blur-up placeholder. */
  width?: number
