/**
 * Sanity-style palette extraction — pure math over the same tiny linear-RGB grid the
 * blurhash encoder samples, so it costs no extra decode. Quantizes the grid into a
 * coarse histogram, then classifies the most populous colors into the seven Vibrant-style
 * swatches (`dominant`, `vibrant`, `muted`, and their dark/light variants), each carrying
 * a contrast-safe text color. Consumers use it for solid-color placeholders behind tiny
 * thumbs, hero text theming, and admin chrome.
 */
import { type LinearGrid, linearToSrgb } from '../blurhash/codec'
