...
// The dev-only gate lives in the plugin (see plugin.spec.ts) — when it's closed these endpoints are
// never registered, so the handlers themselves are unconditional and tested as such.
describe('GET /api/dev', () => {
  ...
  it('redirects browsers to the dev pages, and ?format=json overrides it', async () => {
    const browser = await createDevEndpoint({ devRoute: '/dev' }).handler(request('http://x/api/dev', 'text/html,application/xhtml+xml'))
    expect(browser.status).toBe(307)
    expect(browser.headers.get('location')).toBe('/dev')

    const json = await createDevEndpoint({ devRoute: '/dev' }).handler(request('http://x/api/dev?format=json', 'text/html'))
...
describe('GET /api/dev/stage', () => {
  const stage = () => createStageEndpoint({ devRoute: '/dev' })

  it("sets the selection cookie and redirects to the test's page by default", async () => {
    const res = await stage().handler(request('http://x/api/dev/stage?test=hero&version=bold'))
  ...
  it('honors an explicit `to`', async () => {
    const res = await stage().handler(request('http://x/api/dev/stage?test=hero&version=bold&to=/pricing'))
  ...
  it('clears the cookie with ?clear (and when test/version are missing)', async () => {
    for (const url of ['http://x/api/dev/stage?clear=1', 'http://x/api/dev/stage?test=hero']) {
      const res = await stage().handler(request(url))
  ...
  it('rejects non-path redirect targets (no open redirect)', async () => {
    for (const to of ['https://evil.test', '//evil.test']) {
      const res = await stage().handler(request(`http://x/api/dev/stage?test=a&version=b&to=${encodeURIComponent(to)}`))
      expect(res.headers.get('location')).toBe('/')
    }
  })

  it('targets a chrome-override cookie with ?slot and redirects home (site-wide swap)', async () => {
    const res = await stage().handler(request('http://x/api/dev/stage?slot=header&test=site-header&version=mega'))
...
describe('GET /api/dev/draft', () => {
  it('reports the current state without params', async () => {
    draftState.isEnabled = false
    const res = await createDraftEndpoint().handler(request('http://x/api/dev/draft'))
    expect(res.status).toBe(200)
  ...
  it('flips draft mode with ?enable and reports the new state', async () => {
    draftState.isEnabled = false
    const on = await createDraftEndpoint().handler(request('http://x/api/dev/draft?enable=1'))
    expect(await on.json()).toEqual({ enabled: true })
    expect(draftState.isEnabled).toBe(true)

    const off = await createDraftEndpoint().handler(request('http://x/api/dev/draft?enable=off'))
  ...
  it('400s on an unrecognized enable value', async () => {
    const res = await createDraftEndpoint().handler(request('http://x/api/dev/draft?enable=maybe'))
    expect(res.status).toBe(400)
  })

  it('redirects with `to` (same-site paths only)', async () => {
    const res = await createDraftEndpoint().handler(request('http://x/api/dev/draft?enable=1&to=/pricing'))
    expect(res.status).toBe(303)
    expect(res.headers.get('location')).toBe('/pricing')

    const evil = await createDraftEndpoint().handler(request('http://x/api/dev/draft?to=//evil.test'))
...
describe('POST /api/dev/icons/activate', () => {
  ...
  it('400s without payload-icons or without an id', async () => {
    ...
    expect((await createActivateIconSetEndpoint().handler(noIcons.req)).status).toBe(400)

    const noId = activateRequest({ custom: { payloadIcons: { iconSetSlug: 'iconSet' } } })
    expect((await createActivateIconSetEndpoint().handler(noId.req)).status).toBe(400)
  })
  ...
  it('publishes the chosen set as active', async () => {
    ...
    const res = await createActivateIconSetEndpoint().handler(req)
    expect(res.status).toBe(200)
