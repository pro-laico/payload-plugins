...
`mux-video`, whose bytes are uploaded to Mux rather than stored as a Payload upload. To seed one of those,
mark the collection's own Payload config so the engine hands the `_file` to that hook instead of trying to
upload bytes:
...
- **No circular references.** `ref()` builds a dependency graph, so two docs that reference each other are
  a hard error naming the cycle (`posts:a -> authors:b -> posts:a`). Break the loop — drop one side's `ref`
...
references. A cycle is a hard error that names the cycle.
</Step>
...
value for a `custom.seedAsset` collection's ingest hook.
</Step>
