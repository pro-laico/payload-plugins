...
user and reports success (or the error) inline. When `ENABLE_SEED` isn't set, the button doesn't
render at all — environments where the endpoint would refuse anyway never show it.
...
- **Nothing runs without `ENABLE_SEED`.** The endpoint and CLI no-op (`403`) unless `ENABLE_SEED` is
  exactly `"true"`, and the admin button isn't even rendered. The in-code `seed()` is the exception — it
  skips the gate, so it will wipe whatever database it's handed. Guard your script / test setup accordingly.
