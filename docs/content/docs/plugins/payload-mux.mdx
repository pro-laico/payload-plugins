...
- **Direct-to-Mux upload.** An admin uploader field posts the file straight to Mux via a pair of
  upload endpoints, so the bytes never touch your server or database.
...
hook. Playback policy comes from the plugin's configured policy, so seeded and admin-uploaded videos
share one policy. See the
