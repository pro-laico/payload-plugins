...
- **`iconRequest`** — a hidden diagnostics collection, on by default (disable with `trackRequests: false`).
...
Drafts / versions are on by default; toggle with `iconSetOverrides.drafts` (default `true`). Lives
under a **Sets** admin group.

- **Settings tab:** the `active` toggle and `title` (plus any `fields` you add, and the [usage panel](#icon-use-detection) unless `usagePanel: false`).
