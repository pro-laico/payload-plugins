...
```ts title="src/components/Icon.ts"
...
- **Static scan sees literal names only.** It matches `name="x"`, `name={'x'}`, and <code>{'name={`x`}'}</code>. A dynamic `name={expr}` is invisible to it — that's the runtime recorder's job (the `iconRequest` collection).
