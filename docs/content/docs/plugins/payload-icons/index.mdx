...
- **Revalidation is opt-in.** The icon read caches and tags itself, but on its own the plugin busts nothing — the entry simply lives out its cache lifetime. Install the companion [`@pro-laico/payload-revalidate`](/docs/plugins/payload-revalidate) and any icon or set write invalidates that tag, making the whole surface self-healing — see [How it works](/docs/plugins/payload-icons/how-it-works).
