...
- **Next.js 15+ to render icons.** The collection, the admin, and the SVG pipeline run in any Payload app. `<Icon>` reads `draftMode()` from `next/headers`, and runtime miss-tracking uses `after()` from `next/server` — so *rendering* icons is Next-bound even though managing them isn't. The `next` peer is optional for exactly that reason.
