...
the walk finds it, including inside Lexical richText. It's flagged in dev in the
[`/dev/revalidate` map](/docs/plugins/payload-revalidate/troubleshooting) (and on the
console with `options: { advisories: true }`) as a refactor candidate, with the exact
field paths.
</Callout>
...
| `posts:join:author:7` | an entry that populated a join field — `{child}:join:{on}:{parentId}` | a `posts` doc joins or leaves author `7`, or a field in that join's `where` changes |
...
| the field a join points at (`author`) on a child doc | `posts:join:author:{id}` for the parent it left **and** the one it joined — a delete busts every parent it was on |
| a field in a join's `where` on a child doc | that join's tag for every parent the child is on — the `where` fields are the join's determinants, read off your config |
