...
export const revalidateOptions = [
  ...
  {
    name: 'collections',
    type: 'Partial<Record<string, CollectionRevalidateConfig | false>>',
    default: '{}',
    required: false,
    description:
    ...
    options: [
      {
        ...
        type: 'string',
        default: "''",
        description: 'Namespaces every emitted tag — the separator is added for you (e.g. `prefix: "app"` → `app:posts:my-slug`). Use it to isolate tags when two Payload apps share a cache.',
      },
      {
        name: 'rules',
      ...
      },
      {
        ...
        default: 'false',
        description:
          'Print the dev-only anti-pattern advisories to the console — bake-ins, undeclared list scopes, content-carrying `cacheIds` reads, risky aliases. Off by default: the `/dev/revalidate` map surfaces the same findings without flooding the dev logs on every cache-entry materialization. Hard failures (untagged entries, dropped tags, missing marker) always log regardless.',
      },
      {
        name: 'access',
        type: 'RevalidateAccessOptions',
        default: 'open in non-production, logged-in user in production',
        description:
          "Per-endpoint gate for the map endpoints — one `EndpointAccess` (`(req) => boolean | Promise<boolean>`). Only relevant while `observe` is on; otherwise the endpoints aren't registered. See [Gating endpoints](../conventions#gating-endpoints).",
        options: [
          {
            name: 'inspect',
            type: 'EndpointAccess',
            default: 'open in non-production; requires a logged-in user in production',
            description: 'Gates `GET` / `POST /revalidate-map`. Denials answer `404` to hide that the endpoint exists.',
          },
        ],
      },
    ],
...
Two dependency-map routes, governed entirely by [`observe`](#plugin-options) — dev-only by default. They serve nothing but observations, so with `observe: false` (the production posture) they are not registered at all. When registered, access is `options.access.inspect`: open in non-production, a logged-in Payload user in production. Override it to change who gets in; denials answer `404` to hide the endpoint. See [Gating endpoints](../conventions#gating-endpoints).
...
| Export | Purpose |
| --- | --- |
| `revalidatePlugin` | The plugin, also the default export. Add it **last** in `plugins: []`. |
| `tagsFor` | `tagsFor(payload)` builds prefix-correct raw tags for the rare hand-spelled `cacheTag` need. |
| `readRevalidateMarker` | `readRevalidateMarker(config)` returns the typed `config.custom.payloadRevalidate` — the resolved `prefix`, `observe`, `advisories`, `lists`, `extraTags`, `rules`, and the map `endpointPath` (non-null only while observing). |
...
**Types** — same entry point, `@pro-laico/payload-revalidate`

| Type | What it types |
| --- | --- |
| `RevalidatePluginOptions` | The `revalidatePlugin()` options object. |
| `CollectionRevalidateConfig` · `RevalidateMarker` | One collection's tracking config (`idField`, `lists`, `extraTags`), and the `custom.revalidate` marker a collection ships without importing anything — `CollectionRevalidateConfig \| false`. |
| `DependencyRule` | One `options.rules` entry: `{ on, bust, whenFields? }`. |
| `RevalidateAccessOptions` · `EndpointAccess` | The `options.access` gate-map (`{ inspect? }`) and the gate function `(req) => boolean \| Promise<boolean>`; see [Gating endpoints](../conventions#gating-endpoints). |
| `PayloadRevalidateMarker` | What `readRevalidateMarker(config)` hands back. |
| `Tags` | The tag builders `tagsFor(payload)` returns: `doc`, `list`, `join`, `global`, `all`. |
| `RevalidateInspection` · `InspectedCollection` · `ObservedRead` · `RevalidateEvent` · `ScannedGetter` | The inspection payload and its parts — resolved settings, observed reads, bust events, and scanned getters. |
| `ReferenceGraph` · `ReferenceEdge` | The reference graph inside an inspection: collections, globals, and one edge per reference (`relationship` / `upload` / `join` / `richText`). |
| `MapConfigSource` · `RenderMapOptions` | The config shape `buildStaticInspection` accepts, and `renderRevalidateMap`'s options (`{ live? }`). |
...
- Failure paths are loud in every environment, once per process: `cacheTag` failing
  ...
  to be quiet about. The one exception: a seed's end-of-run flush no-oping in a CLI
  process is expected (no cache lives there), so it stays silent.
