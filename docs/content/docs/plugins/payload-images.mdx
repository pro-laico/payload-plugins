...
<TypeTable
  type={{
    ...
    croppedBlurHash: { description: "The placeholder every read carries: a finished data URI focal-cropped to the render the read declares (req.context.blurhash / X-Blurhash header); the raw sm tier hash when nothing is declared.", type: "virtual" },
...
extra query per image). Disable with `virtualFields: false`. (The `croppedBlurHash` placeholder rides along too — it isn't
...
export async function Hero({ id }: { id: string }) {
  ...
  const image = await payload.findByID({
    ...
    id,
    depth: 0,
    // Declare the render → `croppedBlurHash` arrives as a finished data URI, focal-cropped to this ratio.
...
export const imageProps = [
  ...
  { name: 'placeholder', type: 'boolean', default: 'true', description: `Paint the doc's croppedBlurHash behind the image while it loads (a tiny inline image, zero network, zero client JS). false skips it even when the doc carries one. The quality/crop are decided by the READ that fetched the doc, not here.` },
...
<ResponsiveImage
  ...
  fetchPriority="auto"
  decoding="async"
  placeholder      // on by default; renders the doc's croppedBlurHash — false to skip
  baseUrl=""          // same-origin
  path="/api/img"
  // optional, no default:
...
On read, the virtual `croppedBlurHash` field does **all the placeholder work in the field hook**,
...
const image = await payload.findByID({
  ...
  context: { blurhash: { ar: '16:9', quality: 'sm' } }, // croppedBlurHash arrives as a finished data URI
})
```
...
<Callout type="info">
  **Non-React consumers** get the same placeholder: send `X-Blurhash: 16:9; q=md` on a REST read
  (or set `req.context.blurhash` on a Local API one) and `croppedBlurHash` comes back as a data URI
...
- **It rides along automatically.** The endpoint reads all three layers from the document on any
  ...
  `croppedBlurHash` placeholder all honor them with nothing extra to pass. Non-cover fits honor
  the crop rect.
