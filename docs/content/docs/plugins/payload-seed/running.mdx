...
export const runNodes = [
  ...
  { id: 'validate', label: 'Build + validate the model', detail: 'every ref resolves, no unknown field, no duplicate _key — issues collected and thrown at once (400)', x: 300, y: 330 },
...
- **The caller must pass `options.access.run`**, or `403`. By default that's any authenticated request —
  a Payload auth cookie or API key — and you can narrow it to a role. It never widens the kill switch:
  `ENABLE_SEED` is checked first and independently. See [Gating endpoints](../conventions#gating-endpoints).
...
<Callout type="info">
  **Which `403` is it?** Read the `error` string — the two denials word themselves differently.
  `"Seeding is disabled. Set ENABLE_SEED=true …"` is the kill switch. `"Seeding requires an
  authenticated Payload user …"` is the `access.run` gate turning the caller away.
</Callout>
