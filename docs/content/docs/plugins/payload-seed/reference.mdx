...
export const options = [
  ...
  { name: 'definitions', type: 'SeedDefinition[]', description: `The seed definitions: your defineSeed exports. The same array feeds the seed run and the typed SeedRegistry injected into payload-types.ts. Omit it and the seed runs but warns there is nothing to do. Kept at the root — it's what the plugin is.` },
  {
    ...
    options: [
      ...
      {
        name: 'access',
        type: 'SeedAccessOptions',
        description: `Per-endpoint gates for the plugin's HTTP endpoints — one EndpointAccess ((req) => boolean | Promise<boolean>) per endpoint. See Gating endpoints.`,
        options: [
          { name: 'run', type: 'EndpointAccess', default: 'any logged-in user', description: `Gates POST /seed. Runs after the ENABLE_SEED=true env kill-switch, not instead of it — ENABLE_SEED decides whether the endpoint answers at all, this gate decides who may call it. Both are required.` },
        ],
      },
    ],
  },
...
seedPlugin({
  ...
  options: {
    ...
    // access: {
    //   run: ({ user }) => Boolean(user), // default: any logged-in user (still requires ENABLE_SEED=true)
    // },
  },
})
```
...
| Route | What |
| --- | --- |
| `POST /api/seed` | Run the seed over HTTP. Requires `ENABLE_SEED=true` (the kill-switch, checked first) **and** a caller `options.access.run` admits (any logged-in user by default; see [Gating endpoints](../conventions#gating-endpoints)). See [Running the seed](/docs/plugins/payload-seed/running). |
...
| `SeedAccessOptions` | `@pro-laico/payload-seed` | The `options.access` gate-map shape: `{ run? }`. |
| `EndpointAccess` | `@pro-laico/payload-seed` | The endpoint-gate function type `(req) => boolean \| Promise<boolean>`; see [Gating endpoints](../conventions#gating-endpoints). |
