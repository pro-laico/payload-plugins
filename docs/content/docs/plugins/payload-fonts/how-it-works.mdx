---
title: "How it works"
description: "How the subset pipeline, one-file italics, the baked next/font path, and the PreviewFonts escape hatch fit together — so you can reason about what happens on save, on build, and on refresh."
---
...
For the collections themselves see [Collections](/docs/plugins/payload-fonts/collections); for wiring serving into your layout see [Serving](/docs/plugins/payload-fonts/serving).
...
export const pipelineNodes = [
  ...
  { id: 'download', label: 'predev + prebuild: payload fonts:download', detail: 'reads the active fonts through the Local API (no running site) → writes public/fonts/*.woff2 + definition.ts', x: 40, y: 690 },
  { id: 'build', kind: 'result', label: 'Baked: next/font/local (dev and prod)', detail: 'extractFonts puts the --font-set* classes on <html> — the main path; rebuild to publish a fontSet change', x: 40, y: 880 },
  { id: 'preview', kind: 'result', label: 'Preview: read live', detail: 'PreviewFonts inlines @font-face from the DB at render time — the escape hatch, no rebuild', x: 560, y: 690 },
]
...
export const pipelineEdges = [
  ...
  { from: 'optimized', to: 'preview', label: 'per render', fromSide: 'right' },
]
...
- [Serving](/docs/plugins/payload-fonts/serving) — wire the baked path and the `PreviewFonts` escape hatch into your layout.
- [Troubleshooting](/docs/plugins/payload-fonts/troubleshooting) — stale fonts, blank admin fields, and the wasm-bundling failure.
