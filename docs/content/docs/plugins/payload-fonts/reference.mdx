...
export const options = [
  ...
  { name: 'options', type: 'FontsOptions', description: `The plugin's own knobs.`, options: [
    ...
    { name: 'access', type: 'FontsAccessOptions', description: `Per-endpoint gates for the plugin's HTTP endpoints — one EndpointAccess ((req) => boolean | Promise<boolean>) per endpoint. See Gating endpoints.`, options: [
      { name: 'export', type: 'EndpointAccess', default: 'PAYLOAD_SECRET bearer-token check', description: `Gates GET /fonts/export. A machine endpoint with no user session — defaults to the PAYLOAD_SECRET bearer-token check the download CLI sends. Override only if you terminate auth upstream.` },
    ] },
  ] },
]
