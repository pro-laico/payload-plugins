...
export async function Hero({ id }: { id: string }) {
  ...
  if (!img) return null
  // aspectRatio matches the read; sizes = how big it actually renders
  return <ResponsiveImage {...img} aspectRatio="16:9" sizes="(max-width: 768px) 100vw, 50vw" />
}
```

Declare `aspectRatio` in **two** places and keep them equal: on the read (so the crop is
computed) and on the component (so the CSS box matches the crop). Set `sizes` to how big the
image actually renders, not how big the file is.

The doc comes back as `{ id, alt, src, srcset, placeholder }` — a full responsive `srcset` and
...
The component returns `null` when the doc has no `src`/`srcset` (deleted image,
`virtualFields: false`). A missing image renders nothing rather than a broken frame.
