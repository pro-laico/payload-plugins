...
export const imageProps = [
  ...
  { name: 'placeholder', type: 'string | null', description: `The doc's placeholder — painted as the <img>'s background while it loads when it's a URL-shaped value (data:, http(s):, or root-relative). A raw blurhash string is carried but not painted. The quality/crop were decided by the READ that fetched the doc.` },
