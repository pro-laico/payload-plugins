...
imagesPlugin({
  ...
  options: {
    ...
    prewarm: {
      ...
      strategy: {
        widths: 'srcset', // or { every: 2 } (every 2nd reachable width), or an explicit number[]
...
| `payload images:prewarm` | `--now`, `--limit`, `--collection`, `--queue` | Bulk-warms a library against the learned render profiles. Requires `prewarm` on. See [Prewarming](/docs/plugins/payload-images/prewarming). |
