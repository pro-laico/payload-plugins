...
| Uploads succeed but prewarm jobs pile up and never run (typically on serverless). | The default runner is `strategy.autoRun`, an in-process 5-minute cron — it works in any long-lived process but never fires reliably on serverless. | Schedule the run externally: a cron hitting `/api/payload-jobs/run?queue=images-prewarm` (e.g. a Vercel Cron), or `payload images:prewarm --now` as a build step. See [Prewarming → When it runs](/docs/plugins/payload-images/prewarming#when-it-runs). |
