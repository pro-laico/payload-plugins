...
The `srcset` follows the project-wide `pixelStep` (default `50`): one URL every 50px up to the source width, whose native width is always the top rung. An array `pixelStep` switches to a fixed breakpoint ladder instead (e.g. next/image's `[640, 750, 828, 1080, 1200, 1920, 2048, 3840]`).
...
export const virtualNodes = [
  ...
  { id: 'hook', label: 'afterRead computes the virtual fields', detail: 'on every read, never stored — srcset steps by pixelStep up to the source width', x: 300, y: 150 },
...
- **`v=1a2b3c`** — a cache-bust token derived from the source's filename + focal point. The server
  ...
  ([Caching](/docs/plugins/payload-images/caching)).
