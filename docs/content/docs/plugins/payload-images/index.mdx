...
- **Focal-point cropping, subject-aware by default:** uploads get a saliency-detected focal point when the analysis finds a subject and the editor hasn't set one; refine it once and [every crop, at any ratio, stays centered on it](/docs/plugins/payload-images/metadata#focal-point-hotspot--crop).
...
- **Instant placeholders, cropped like the image:** [a ladder of quality tiers stored at upload](/docs/plugins/payload-images/metadata); every read gets a finished placeholder focal-cropped to the exact ratio being rendered — `sm` by default, `.blur(tier)` to pick, `blur: false` to opt out.
- **Self-busting cache, smart prewarming, instant fallbacks:** URLs refresh themselves on change, [prewarming](/docs/plugins/payload-images/prewarming) generates what your site actually serves before anyone asks, and a cold request serves a [nearby stand-in instantly](/docs/plugins/payload-images/caching#nearby-quality-fallback).
...
<Cards>
  ...
  <Card href="/docs/plugins/payload-images/caching" title="Caching">
    Content-addressed caching, self-revalidation, abuse limits, and the nearby-quality fallback.
  </Card>
  <Card href="/docs/plugins/payload-images/prewarming" title="Prewarming">
