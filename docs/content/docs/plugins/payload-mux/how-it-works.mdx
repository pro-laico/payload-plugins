...
export const lifecycleNodes = [
  ...
  { id: 'heal', label: 'Or: a save refetches it', detail: 'no webhook? saving a doc with an assetId but no playbackOptions asks Mux again', x: 860, y: 430 },
...
export const lifecycleEdges = [
  ...
  { from: 'webhook', to: 'heal', label: 'never arrived', dashed: true, fromSide: 'right' },
  { from: 'inline', to: 'urls' },
...
server-side ingest. Fetch the video doc through an id-keyed getter and derive the
...
export async function getVideo(id: string | number) {
  'use cache'
  return findDocByID('mux-video', id)
}
```
...
**custom ingestion**. `muxPlugin()` marks the collection with
...
plugins: [muxPlugin(), seedPlugin({ definitions: [videos, pages] })]
```
