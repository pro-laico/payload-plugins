  ...
  },
  {
    ...
    options: [
      ...
      { name: 'playbackPolicy', type: "'public' | 'signed'", default: "'public'", description: `Playback policy for newly uploaded videos. Shorthand for uploadSettings.new_asset_settings.playback_policy; set 'signed' (plus MUX_SIGNING_KEY / MUX_PRIVATE_KEY) for JWT-signed URLs.` },
      {
        ...
        description: `Per-endpoint gates for the plugin's HTTP endpoints — one EndpointAccess ((req) => boolean | Promise<boolean>) per endpoint. Collection reads and writes are gated the Payload way, under collections.muxVideo.overrides.access — the collection ships read: authd (any logged-in user). Keep it closed under a signed playback policy: playback URLs are JWT-signed on read, so an anonymous read hands out a working signed URL. See Gating endpoints.`,
        options: [
          { name: 'upload', type: 'EndpointAccess', default: 'any logged-in user', description: `Gates the direct-upload endpoints (POST / GET /mux/upload).` },
          { name: 'webhook', type: 'EndpointAccess', default: 'Mux signature verification', description: `Gates POST /mux/webhook. Defaults to verifying the Mux webhook signature; override only if you terminate that check upstream.` },
          { name: 'refresh', type: 'EndpointAccess', default: 'any logged-in user', description: `Gates GET /mux/refresh, which asks Mux whether a document's asset finished encoding. The admin uploader polls it while a video is preparing, so a doc whose ready webhook never arrived still resolves itself.` },
        ],
      },
...
(default Mux signature verification) — see [Gating endpoints](../conventions#gating-endpoints). See
[Endpoints & webhooks](/docs/plugins/payload-mux/endpoints-and-webhooks) for the webhook setup and
...
| `muxVideoPlugin` | `@pro-laico/payload-mux` | **Deprecated** alias for `muxPlugin`, kept for one release — removed in 0.6.0. |
...
| `MuxAccessOptions` | `@pro-laico/payload-mux` | The `options.access` gate-map shape: `{ upload?, webhook? }`. |
| `EndpointAccess` | `@pro-laico/payload-mux` | The endpoint-gate function type `(req) => boolean \| Promise<boolean>`; see [Gating endpoints](../conventions#gating-endpoints). |
