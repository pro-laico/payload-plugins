...
Under a **signed** policy each read signs all three URLs (`playbackUrl` / `posterUrl` / `gifUrl`),
whether or not you render them — the collection's `forceSelect` keeps `playbackOptions` whole, so a
narrower `select` doesn't skip any of them. Public playback does no signing.
