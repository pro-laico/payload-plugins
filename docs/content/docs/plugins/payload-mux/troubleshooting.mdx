...
| **Boot fails: `extendCollection: '<slug>' already defines field(s) …`** | You pointed `extendCollection` at a collection that already has a field named like one the plugin injects: `muxUploader`, `source`, `title`, `assetId`, `status`, `error`, `duration`, `posterTimestamp`, `aspectRatio`, `maxWidth`, `maxHeight`, `playbackOptions`. Fields append on merge, so two same-named fields would otherwise reach Payload as a bare `DuplicateFieldName`. A colliding field nested in a row, collapsible, or unnamed tab counts too — those share the parent's level. | Rename or remove your field, or drop `extendCollection` and let the plugin register its own `mux-video` collection. |
| **Boot fails: `extendCollection: collection '<slug>' not found`** | `extendCollection` runs when the plugin is applied, so the target must already be in `collections`. | Check the slug, and that the collection is registered in `payload.config.ts` (a plugin that adds the collection must run before this one). |
