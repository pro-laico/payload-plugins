...
- **An [`<Icon>` component](#icon):** resolves a name through the active set and inlines a real, recolorable `<svg>`.
- **An [`icon` collection](#icon-1):** SVG-only uploads, optimized + sanitized + themed on save.
...
- **[Icon usage detection](#icon-usage-detection):** the **Requested icons** panel scans your code (live in dev) plus tracks runtime misses, surfacing which icons you need and which a set is still missing.
...
[`@pro-laico/payload-seed`](/docs/plugins/payload-seed)'s normal `asset()` flow, with no plugin-specific
