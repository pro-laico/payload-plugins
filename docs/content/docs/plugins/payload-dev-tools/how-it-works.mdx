...
- **Gating is layered.** `enabled` is a *config-time* gate: when it's off the plugin hands back the
  config untouched, so the `/api/dev*` endpoints are never registered — they don't exist rather than
  404 per request. The pages and toolbar check the same `enabled` value per render. The one
  per-request check is `options.access.dev`, and it covers the **endpoints only** — the `/dev` pages
  are gated by `enabled` alone. A production build ships neither markup nor data.
