...
| Route | What you see |
| --- | --- |
| `/dev` | Env + plugin chips, seed card (seed / destructive reseed with errors inline), the environment card (which env file booted, database host, missing variables), doc counts. |
...
`env` reports presence, never values — `set` booleans and the database host with its credentials
stripped. See [Environments](/docs/plugins/payload-dev-tools/environments).

## Next

- [The test harness](/docs/plugins/payload-dev-tools/test-harness) — build page/header/footer variants and flip between them.
- [Testing by location](/docs/plugins/payload-dev-tools/regions) — the region override and the consent model it resolves to.
- [Environments](/docs/plugins/payload-dev-tools/environments) — `payload-dev-env` and the env diagnostics in the snapshot.
