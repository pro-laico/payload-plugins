...
```ts
// src/seed/fonts.ts: a variable typeface (one file, every weight) and a static two-weight pairing
import { defineSeed } from '@pro-laico/payload-seed'
...
export default defineSeed('font', ({ ref }) => [
  {
    ...
    title: 'Inter',
    family: 'sans',
    // variable font: one file covers the whole weight axis (100–900), served with its range intact
    variable: { upright: ref('fontOriginal', 'inter-variable') },
  },
  {
    _key: 'lora',
    title: 'Lora',
    family: 'serif',
    // static files: one row per weight you actually use — the classic body + bold pairing
    weights: [
