...
- **A `fontSet` global.** Choose which uploaded typeface fills each family; that selection is the
...
Besides the admin slots, a typeface can be created **server-side** from a local file or URL, which is
handy for imports, migrations, and seeding. `ingestFont` hands the file to the `font` collection as a
