...
generated, so **gitignore** `src/app/definition.ts` and `public/fonts/` — no file to hand-create
([reference](#serving)):
...
`definition.ts` and `public/fonts/` are generated — gitignore them. A `predev: payload-fonts-download`
