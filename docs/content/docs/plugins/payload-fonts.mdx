...
- **Upload a font, get an optimized one.** Drop a typeface into the admin and the plugin
  automatically shrinks it to a lean, web-ready file.
- **Pick your fonts in the CMS.** Choose which typeface is your sans, serif, mono, and display —
  swap them anytime, no code change.
- **Your site self-hosts them.** Fonts are served from your own app — fast and private, no Google
  Fonts or third-party CDN — the same in development and production. Changing fonts in production
  takes a rebuild ([why](#revalidation--caching)).
...
Production self-hosts via `next/font/local`. Run the download before the build and commit an empty
`definition.ts` so the import resolves on a fresh checkout ([reference](#serving)):
...
`prebuild` authenticates against your running Payload instance, so set two env vars:
...
If you customise families, pass the same `families` to `<DevFonts>` so the dev preview reads the same
slots and fallbacks.
...
export const devFontsProps = [
  ...
  { name: 'families', type: 'FontFamilyConfig[]', default: 'sans / serif / mono / display', description: `Pass the same families you gave fontsPlugin, if you customised them — so the dev preview reads the same slots and per-family fallbacks. Omit it when using the default families.` },
]
...
// seed/fontSet.ts — pick the active typeface per family with ordinary ref() tokens
...
| `ingestFont` | `@pro-laico/payload-fonts` | Create a `font` typeface from a local file / URL (imports, migrations). |
| `fontAssetProvider` | `@pro-laico/payload-fonts` | Register `font` as a `@pro-laico/payload-seed` asset provider. |
| `fontSource` | `@pro-laico/payload-fonts` | Declare a seeded typeface's file (a seed source token). |
