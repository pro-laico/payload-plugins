...
- **Server ingest + declarative seeding:** create a typeface from a file or URL in code with `ingestFont`, or seed one like any other doc through [`@pro-laico/payload-seed`](/docs/plugins/payload-seed).
...
the `font` collection's hooks (`getFontSourceHook` → `optimizeFromOriginals`), so there's no separate
build step to run and nothing to keep in sync by hand.
