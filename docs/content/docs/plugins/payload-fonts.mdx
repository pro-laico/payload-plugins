...
export const fontOriginals = defineSeed('fontOriginal', ({ file }) => [
  {
    _key: 'inter',
    _file: file('inter.woff2'), // assets/font/inter.woff2 (via assetSubDirs)
  },
])
```
...
- **A bundled subsetter fails silently.** Without `serverExternalPackages: ['subset-font',
  'harfbuzzjs', 'fontkit']` in next.config, Next bundles the subsetter, the wasm breaks, and
  optimization silently skips — fonts save but never shrink. It's a Quickstart step for a reason.
