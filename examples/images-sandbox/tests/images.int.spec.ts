...
describe('payload-images wiring', () => {
  ...
  it('<ResponsiveImage> renders a single <img> (Shape B) painting the render-ready doc it was handed', async () => {
    ...
    expect(doc.srcset).toMatch(/w=650&h=366/) // 16/9 h derived per 50px grid step, not the natural ratio
  ...
  it('getImageUrl builds an absolute, focal-cropped OG URL that the endpoint serves (the generateMetadata pattern)', async () => {
    ...
    // And it actually resolves to an image (height snaps to the anti-DoS 50px grid: 630 → 650;
    // the og PRESET serves exact 1200×630 — this freeform URL deliberately takes the snap).
    const handler = getHandler(payload, 'get', '/img/:id')
