...
const originalExists = async (id: Id): Promise<boolean> => {
  try {
    await payload.findByID({ collection: 'fontOriginal' as never, id })
    return true
  } catch {
    return false
  }
...
const createFont = (data: Record<string, unknown>) =>
  payload.create({ collection: 'font', data } as Parameters<typeof payload.create>[0]) as unknown as Promise<Doc>
...
describe('font collection hooks (integration)', () => {
  ...
  it('cascades delete to optimized + original files (beforeDelete, relationship still intact)', async () => {
    ...
    await payload.delete({ collection: 'font', id: font.id as Id } as unknown as Parameters<typeof payload.delete>[0])
  ...
  it('getActiveFontFaces resolves the fontSet selection to served faces', async () => {
    ...
    await payload.updateGlobal({ slug: 'fontSet', data: { sans: font.id } } as Parameters<typeof payload.updateGlobal>[0])
  ...
  it('populates a font relationship leanly — defaultPopulate omits the private upload slots', async () => {
    ...
    await payload.updateGlobal({ slug: 'fontSet', data: { display: font.id } } as Parameters<typeof payload.updateGlobal>[0])

    // Traverse fontSet → font; `defaultPopulate` returns identifying metadata only.
    const fontSet = (await payload.findGlobal({ slug: 'fontSet', depth: 1 })) as { display?: Record<string, unknown> }
