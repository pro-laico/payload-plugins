 ...
 *  (with a new `v=` token, so the derived binary variants refresh too). */
export const getImage = async (id: string | number): Promise<MediaImage | null> => {
  'use cache'
  const payload = await getPayloadClient()
  const doc = (await payload.findByID({ collection: 'images', id, depth: 0, disableErrors: true, overrideAccess: true })) as MediaImage | null
