...
| `tests/images.int.spec.ts` | payload-images | boot guards (`extendCollection`/`sourceSlug` misconfig → thrown), boot warnings (`img` slug shadowing, virtualFields-without-transform), transform endpoint 400/404, missing source bytes (502), corrupt source bytes (500) — clients get terse bodies by design; the diagnosis (id + filename + cause) is asserted in the LOG. Purge 401/400 |
